...
data_transforms = {  #이미지 막 변형
    'train': transforms.Compose([
        ...
        transforms.RandomRotation(15), # -15 ~ +15도 사이로 회전
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ]),
    'val': transforms.Compose([
        transforms.Resize(224), # 크기 변경
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ]),
}
...
print("GPU_number : ", "7", '\tGPU name:', torch.cuda.get_device_name(torch.cuda.current_device()))

save_path='model/VGG16_False'
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in tqdm(range(num_epochs)):
        # print('Epoch {}/{}'.format(epoch, num_epochs - 1)) #왜 num-1이 99출력해?->밑에 100잇서, 호출기준
        # print('-' * 10)
        ...
        for phase in phases:
            ...
            savepath = save_path + '/new_{:.6f}_L1_{:.6f}_E_{}.pth' #.pth없으면 권한오류
